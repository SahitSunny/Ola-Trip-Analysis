...
select count(*) from trips;	# only contains successful trips
select count(*) from trips_details; 	# contains cancelled trips also
 
# total trips in a day
select count(distinct tripid) from trips_details;

select tripid, count(tripid) cnt from trips_details	# no duplicate tripID
group by tripid
...
# total number of complete trip
select count(distinct tripid) trips from trips;	# as trip contains complete ride data

# total number of searches
select sum(searches) searches from trips_details;

# total number of searches which got estimated
...
select count(*) - sum(driver_not_cancelled) seaches from trips_details;
...
# two locations which have most number of trips
select * from
...
# which duration got highest trips in each of the location
select * from
...
# which are got the heighest fares, cancellations and trips
...
# which duration got highest trips and fare
select * from	
(select *, rank() over(order by fare desc) rnk
from
